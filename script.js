...
createCard({name: "Мусик", img_link: "https://www.friendforpet.ru/api/sites/default/files/2022-01/%D0%BB%D0%B5%D0%B2%D0%B83_%D0%B0%D0%BB%D0%B5%D0%BA%D1%81.jpg"}, container);

fetch(`https://sb-cats.herokuapp.com/api/2/grimi_tiny/show`) 
...
const addCat = function(cat) {
	fetch(`https://sb-cats.herokuapp.com/api/2/grimi_tiny/add`, {
		method: "POST",
		headers: { 
...
const deleteCat = async function(id, tag) {
	...
	let res = await fetch(`https://sb-cats.herokuapp.com/api/2/grimi-tiny/delete/${id}`, {
		method: "DELETE"
	});
...
const updCat = async function(obj, id) {
	let res = await fetch(`https://sb-cats.herokuapp.com/api/2/grimi-tiny/update/${id}`, {
		method: "PUT",
		headers: {
			"Content-Type": "application/json",
		},
